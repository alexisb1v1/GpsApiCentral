...
export class UpdateUserHandler implements ICommandHandler<UpdateUserCommand> {
  ...
  async execute(command: UpdateUserCommand): Promise<Result<UserEntity, AppError>> {
    ...
    if (command.isActive !== undefined) user.isActive = command.isActive;
    ...
    if (saveResult.isOk()) {
      ...
      this.auditService.createLog({
        ...
        entityId: user.id,
        oldValues: oldValues,
        newValues: { id: user.id, name: user.name, email: user.email, role: user.role, isActive: user.isActive },
